<template>
  <div class="form-designer">
    <el-container>
      ...
      <!-- 保存 -->
      <el-dialog title="保存" :visible.sync="saveVisible">
        <el-form :model="saveForm">
          <el-form-item label="表单ID" label-width="80px">
...
import fields from "./formFieldsConfig.js";
import WidgetForm from "./WidgetForm";
import FormConfig from "./FormConfig";
import WidgetFormPreview from "./WidgetFormPreview";
...
export default {
  ...
  created() {
    ...
    this.saveForm.name = this.formName;
  ...
  methods: {
    ...
    // 预览 - 弹窗
    async handlePreview() {
      console.dir("widgetForm", this.widgetForm);
      if (!this.widgetForm.column || this.widgetForm.column.length == 0)
        this.$message.error("没有需要展示的内容");
      else {
        this.widgetFormPreview = await this.transformToAvueOptions(
          this.widgetForm
        );
        this.previewVisible = true;
        console.dir("widgetFormPreview", this.widgetFormPreview);
      }
    },
    // 导入JSON - 弹窗 - 确定
    ...
    handleSave() {
      ...
      setTimeout(() => {
        ...
        console.log("saveVisible", this.saveVisible);
      });
    },
    handleSaveConfirm() {
      console.log("handleSaveConfirm");
      this.transformToAvueOptions(this.widgetForm).then((data) => {
        console.log("data", data);

        let formsStr = localStorage.getItem("forms");

        let forms = JSON.parse(formsStr || "[]") || [];
        if (forms.length) {
          let index = forms.findIndex((item) => item.id == this.formId);
          if (index > -1) {
            forms[index].data = data;
            forms[index].name = this.saveForm.name;
    ...
    // 预览 - 弹窗 - 确定
    handlePreviewSubmit() {
      console.log("$refs.form", this.$refs.form);
