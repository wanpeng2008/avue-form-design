...
export default {
  ...
  methods: {
    ...
    handleRowConfig(row) {
      ...
      this.$router.push({ name: "formDesign", params: { id: row.id } });
    },
    async fetchData() {
      let forms = localStorage.getItem("forms");
      if (forms) {
        let data = JSON.parse(forms);
        if (data && data.length) return data;
      }
      let { data } = await axios.get("/mock/forms.json");
      localStorage.setItem("forms", JSON.stringify(data));
      return data;
    },
    add(newItem) {
      this.list.unshift(newItem);
      localStorage.setItem("forms", JSON.stringify(this.list));
      this.loadData();
    },
